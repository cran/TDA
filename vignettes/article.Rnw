...
The \code{alphaComplexDiag} function computes the persistence diagram of the alpha complex filtration built on top of a point cloud, using the C++ library \href{ https://project.inria.fr/gudhi/software }{\pkg{GUDHI}}. Currently the function \code{alphaComplexDiag} only works on 64-bit R.
...
\begin{center}
<<eqAlphaComplex3, echo=TRUE, eval=TRUE, fig=TRUE, height=2.8, width=2.8>>=
